...
def disease_info_rag_agent(client, user_input: str) -> str:
    """
    Local RAG pipeline over a tiny curated corpus inspired by Medline/WHO-style summaries.
    1. Retrieve the most relevant educational leaflets from LOCAL_RAG_DOCS.
    2. Ask the LLM to answer using ONLY that retrieved context and general knowledge,
       without inventing prescriptions or dosages.
    """
    # 1) Retrieve top documents
    retrieved = rag_retrieve(client, user_input, k=3)
    ...
    system_prompt = (
        ...
        "You are given a small library of trusted, educational medical leaflets.\n"
    ...
    st.session_state.trace.append(
        {
            ...
            "output": {"retrieved_docs": retrieved},
        }
    )
    return answer


def report_summary_agent(client, user_input: str) -> str:
    """
    Summarizes lab or imaging reports in easy-to-understand language.
    """
    system_prompt = (
        "You are a medical report summarization assistant.\n"
        "Given a raw clinical report or description, provide:\n"
    ...
    return llm_chat(client, system_prompt, user_input)
...
def multi_agent_pipeline(user_input: str) -> str:
    ...
    agent_outputs: Dict[str, Any] = {"intent": intent, "member_info": member_info}
    final_answer = ""
    ...
    if intent == "onboarding":
        ...
        appointment_out = appointment_scheduler_agent(client, user_input, patient_key, specialty)
        ...
        info = disease_info_rag_agent(client, user_input)
        agent_outputs["disease_info"] = info
        final_answer = info

    elif intent == "report_summary":
        summary = report_summary_agent(client, user_input)
        ...
        final_answer = llm_chat(client, system_prompt, user_input)
...
def render_appointments_tab():
    ...
    for appt in st.session_state.appointment_db:
        with st.expander(
...
def main():
    ...
    st.caption(
        ...
        "records, disease info retrieval (local RAG), and report summarization.\n\n"
    ...
    st.session_state.default_specialty = st.sidebar.selectbox(
        "Default Specialty for Appointment Requests",
    ...
    if st.sidebar.button("Clear Session State"):
        for key in ["chat_history", "patient_db", "appointment_db", "logs", "trace", "rag_index"]:
            if key in st.session_state:
                del st.session_state[key]
        st.experimental_rerun()

    tab_chat, tab_appts, tab_metrics, tab_debug = st.tabs(
        ["💬 Chat Assistant", "📅 Appointments", "📊 Metrics & Logs", "🛠 Debug Trace"]
    )
